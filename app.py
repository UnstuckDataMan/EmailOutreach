...
def find_linkedin_column(df: pd.DataFrame) -> str | None:
    """Find a LinkedIn URL column from common headers (case-insensitive; ignores spaces/underscores)."""
    candidates = { "personlinkedinurl", "usersocial" }
    for col in df.columns:
        if norm_key(col) in candidates:
            return col
    return None
...
if run:
    ...
    for i in range(len(df)):
        ...
        if linkedin_col:
            lv = row.get(linkedin_col, "")
            out_linkedin_url.append("" if pd.isna(lv) else str(lv))
        else:
            out_linkedin_url.append("")
